...
# ... [existing code] ...
    ...
    with tab_settings:
        ...
        st.info(
            ...
            "thresholds, OCR settings, and database backups."
        )
    with tab_history:
        update_page_title("History")
        render_history_view()
    # ══ TAB 10: SECURITY AUDIT LOGS ═════════════════════════════════════════════
    # Issue #2810: Audit Logs UI has been moved to app/pages/3_Audit_Logs.py
    with tab_audit:
        update_page_title("Security Audit Logs")
        st.subheader("📜 System Security Audit Trail")
        st.info(
            "👈 **Audit Logs have been moved to the dedicated Audit Logs page.** "
            "Please use the sidebar navigation to access the paginated security audit trail."
        )

...
from src.core.anomaly_detector import (
    AnomalyDetector, AnomalyConfig, AnomalyType, AnomalySeverity
)
...
def _render_configuration():
    ...
    with st.form("anomaly_config"):
        ...
        with col2:
            ...
            cluster_threshold = st.slider("Cluster Similarity Threshold", 0.70, 0.99, 0.85, 0.01)
            collusion_threshold = st.slider("Collusion Threshold", 0.70, 0.99, 0.80, 0.01)
...
def _render_detection():
    ...
    if st.button("🚀 Run Anomaly Detection", type="primary", use_container_width=True):
        if use_sample or (uploaded_files and len(uploaded_files) >= 3):
            ...
            st.success(f"✅ Detection complete! Found **{len(result.anomalies)}** anomalies.")
        else:
...
def _render_results():
    ...
    if severity:
        ...
        fig_sev = px.bar(
            x=list(severity.keys()), y=list(severity.values()),
            color=list(severity.keys()),
            color_discrete_map={"critical": "#ef4444", "high": "#f97316", "medium": "#eab308", "low": "#84cc16", "info": "#60a5fa"},
    ...
    if by_type:
        ...
        fig_type = px.pie(values=list(by_type.values()), names=list(by_type.keys()), title="Anomalies by Type")
    ...
    if data["anomalies"]:
        ...
        for i, anomaly in enumerate(data["anomalies"][:15]):
            sev_colors = {"critical": "#ef4444", "high": "#f97316", "medium": "#eab308", "low": "#84cc16", "info": "#60a5fa"}
            ...
            with st.expander(
                ...
                if anomaly["affected_documents"]:
                    st.markdown(f"**Affected Documents:** {', '.join(anomaly['affected_documents'])}")
    ...
    with col1:
        if data["anomalies"]:
            ...
            st.download_button("Download JSON", json.dumps(data, indent=2, default=str),
                "anomaly_results.json", "application/json")
    with col2:
        if data["anomalies"]:

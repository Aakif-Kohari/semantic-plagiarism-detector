...
def render_ai_detection_report(results: Dict[str, AIDetectionResult]) -> None:
    ...
    col4.metric("Avg Perplexity", f"{sum(r.perplexity_score for r in results.values())/total:.2f}")
    ...
    if suspicious_docs:
        ...
        for doc in suspicious_docs:
            result = results[doc]
            with st.expander(f"🤖 {doc} - {result.ai_probability*100:.1f}% AI", expanded=False):
                st.markdown(f"**AI Probability:** {result.ai_probability*100:.1f}%")
                st.markdown(f"**Perplexity:** {result.perplexity_score*100:.1f}% (lower = more AI)")
                st.markdown(f"**Burstiness:** {result.burstiness_score*100:.1f}% (lower = more AI)")
                st.markdown(f"**Pattern Score:** {result.pattern_score*100:.1f}% (higher = more AI)")
                st.markdown(f"**Sentence Variability:** {result.sentence_variability*100:.1f}% (lower = more AI)")

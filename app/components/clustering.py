...
class SemanticClusterer:
    ...
    def _find_optimal_clusters_kmeans(self, embeddings: np.ndarray,
                                      min_clusters: int = 2,
                                      max_clusters: int = 10) -> int:
        """
    ...
    def _compute_metrics(self, similarity_matrix: np.ndarray):
        ...
        for cluster_id in range(n_clusters):
            ...
            if len(indices) > 1:
                ...
                mean_sim = (np.sum(cluster_mat) - len(indices)) / (len(indices) * (len(indices) - 1))
        ...
        self.cluster_metrics = {
            ...
            'average_inter_similarity': np.mean(inter_cluster_sims) if inter_cluster_sims else 0.0,
            'cluster_separation': np.mean(inter_cluster_sims) - np.mean(cluster_sims) if inter_cluster_sims else 0.0
        }
    ...
    def get_suspicious_clusters(self, similarity_threshold: float = 0.70) -> List[int]:
        ...
        suspicious = []
        for cluster_id, intra_sim in enumerate(self.cluster_metrics.get('intra_cluster_similarities', [])):
            if intra_sim > similarity_threshold and self.cluster_metrics['cluster_sizes'][cluster_id] >= 2:
...
class TopicExtractor:
    ...
    def __init__(self, n_topics: int = 10, max_features: int = 1000,
                 method: str = 'nmf', random_state: int = 42):
        """
    ...
    def get_document_topics(self, threshold: float = 0.2) -> Dict[str, List[Tuple[int, float]]]:
        """
        ...
        for doc_idx, doc_dist in enumerate(self.doc_topic_dist):
            ...
            topics = [(topic_idx, float(score)) 
                      for topic_idx, score in enumerate(doc_dist) 
                      if score > threshold]
            doc_topics[f"Document_{doc_idx}"] = sorted(topics, key=lambda x: x[1], reverse=True)
...
class PatternEvolutionTracker:
    ...
    def add_scan(self, similarity_matrix: np.ndarray, document_names: List[str],
...
def plot_cluster_dendrogram(distance_matrix: np.ndarray, 
    ...
    if not CLUSTERING_AVAILABLE:
        ...
        ax.text(0.5, 0.5, "Clustering dependencies not available", 
...
def plot_cluster_scatter(embeddings: np.ndarray, 
                        labels: np.ndarray,
                        document_names: List[str],
                        method: str = 'tsne',
...
def plot_cluster_similarity_heatmap(similarity_matrix: np.ndarray,
                                   labels: np.ndarray,
                                   document_names: List[str],
...
def render_clustering_tab(similarity_matrix: np.ndarray,
                         document_names: List[str],
    ...
    if not CLUSTERING_AVAILABLE:
        st.error("Clustering dependencies not available. Please install scikit-learn and scipy.")
    ...
    if st.button("🔬 Run Clustering Analysis", type="primary"):
        with st.spinner("Performing hierarchical clustering..."):
            ...
            col3.metric("Davies-Bouldin Score", f"{metrics['davies_bouldin_score']:.3f}")
            col4.metric("Avg Intra-Cluster Similarity", f"{metrics['average_intra_similarity']:.3f}")
            ...
            if suspicious:
                ...
                for cluster_id in suspicious:
                    cluster_members = clusterer.get_cluster_membership(document_names)[cluster_id]
                    st.markdown(f"**Cluster {cluster_id}** ({len(cluster_members)} docs): {', '.join(cluster_members[:5])}{'...' if len(cluster_members) > 5 else ''}")
            else:
            ...
            if st.button("🗺️ Show Cluster Similarity Heatmap", key="show_cluster_heatmap"):
...
def render_topic_analysis_tab(document_chunks: Dict[str, List[str]]):
    ...
    if not CLUSTERING_AVAILABLE:
        st.error("Topic modeling dependencies not available. Please install scikit-learn.")
        return
    ...
    if st.button("📊 Extract Topics", type="primary"):
        with st.spinner("Extracting topics from document chunks..."):
            ...
            st.metric("Topic Coherence Score", f"{coherence:.3f}", 

        ...
        if doc_name in self.versions:
            ...
            if version_id <= len(versions):
                ...
                if self.current_versions.get(doc_name) == version_id:
                    self.current_versions[doc_name] = len(versions) if versions else None
                return True
        return False
    ...
    def detect_changes(self, old_content: str, new_content: str) -> Dict:
        ...
        if total_lines > 0:
            changes['change_percentage'] = (
                (changes['total_added'] + changes['total_deleted'] + changes['total_replaced']) / 
                total_lines
            ) * 100
...
class VersionDiffGenerator:
    ...
    def generate_html_diff(self, old_content: str, new_content: str, 
                          old_ver: int = 1, new_ver: int = 2) -> str:
        ...
        for opcode, i1, i2, j1, j2 in opcodes:
            if opcode == 'equal':
                ...
                for line in old_lines[i1:i2]:
                    diff_lines.append(f'<div class="diff-line replace-old">- {line}</div>')
                # Show new lines as added
                for line in new_lines[j1:j2]:
                    diff_lines.append(f'<div class="diff-line replace-new">+ {line}</div>')
...
class PlagiarismEvolutionAnalyzer:
    ...
    def analyze_plagiarism_evolution(self, doc_name: str) -> Dict:
        ...
        for i in range(len(versions) - 1):
            ...
            change_data = {
                ...
                'change_percentage': ((new_sim - old_sim) / old_sim * 100) if old_sim > 0 else 0,
        ...
        analysis['risk_score'] = min(risk_factors / 5 * 100, 100)  # Normalize to percentage
    ...
    def get_similarity_trend_analysis(self, doc_name: str) -> Dict:
        ...
        last_timestamp = x[-1]
        forecast_timestamps = [last_timestamp + (i + 1) * 86400 for i in range(3)]  # 1 day increments
        ...
        trend_analysis = {
            ...
            'magnitude': abs(slope),
            'forecast': {
                'timestamps': [datetime.fromtimestamp(ts).isoformat() for ts in forecast_timestamps],
        ...
        if slope > 0.01:
            ...
            trend_analysis['recommendation'] = 'This document shows increasing similarity scores. Consider reviewing for potential plagiarism.'
        elif slope < -0.01:
            trend_analysis['risk'] = 'low'
            trend_analysis['recommendation'] = 'Similarity scores are decreasing, which is generally positive.'
        else:
            trend_analysis['risk'] = 'medium'
            trend_analysis['recommendation'] = 'Similarity scores are stable. Monitor for any sudden changes.'
...
class SmartChangePatternDetector:
    ...
    def detect_changes(self, old_content: str, new_content: str) -> Dict:
        ...
        scores['unusual_timing_detected'] = self._detect_unusual_timing(old_content, new_content)
    ...
    def _get_recommendations(self, patterns: List[str]) -> List[str]:
        ...
        if 'large_addition' in patterns:
            recommendations.append("Large text additions detected. Verify if content is original.")
        
        if 'massive_deletion' in patterns:
            recommendations.append("Large text deletions detected. Ensure content wasn't removed to hide plagiarism.")
        
        if 'major_replacement' in patterns:
            recommendations.append("Major content replacement detected. Review for potential external copying.")
        
        if 'copy_paste_detected' in patterns:
            recommendations.append("Copy-paste pattern detected. Verify source of new content.")
        
        if not recommendations:
            recommendations.append("No suspicious patterns detected. Document changes appear normal.")
...
class VersionStorageManager:
    ...
    def save_version_manager(self, version_manager: VersionManager, 
                            doc_name: str) -> bool:
...
def render_version_control_ui(version_manager: VersionManager, doc_name: str):
    ...
    sim_scores = [v.similarity_score for v in versions if v.similarity_score is not None]
    col4.metric("Latest Similarity", f"{sim_scores[-1]*100:.1f}%" if sim_scores else "N/A")
    
    # Version timeline
    st.subheader("📊 Version Timeline")
    timeline_df = version_manager.get_version_timeline(doc_name)
    if not timeline_df.empty:
        st.line_chart(timeline_df.set_index('Timestamp')[['Word Count', 'Similarity Score']])
    
    # Version selector
    st.subheader("📑 Version Selector")
    version_options = [f"v{v.version_id} - {v.timestamp.strftime('%Y-%m-%d %H:%M')} ({v.word_count} words)" 
                       for v in versions]
    ...
    with col1:
        ...
        st.markdown(f"- **Created**: {selected_version.timestamp.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        if selected_version.similarity_score is not None:
            st.markdown(f"- **Similarity Score**: {selected_version.similarity_score*100:.1f}%")
    ...
    with col2:
        ...
        if selected_version.change_summary:
            st.markdown(f"**Change Percentage**: {selected_version.change_summary.get('change_percentage', 0):.1f}%")
    ...
    with col1:
        if st.button("📥 Restore This Version", key=f"restore_{doc_name}"):
            content = version_manager.restore_version(doc_name, selected_version.version_id)
            if content:
                st.success(f"✅ Version {selected_version.version_id} restored successfully!")
                st.rerun()
    ...
    with col3:
        if len(versions) > 1 and st.button("🗑️ Delete This Version", key=f"delete_{doc_name}"):
...
def render_plagiarism_evolution_ui(version_manager: VersionManager, doc_name: str):
    ...
    col2.metric("Trend", analysis['trend'].title(), 
                delta=f"{analysis['metrics']['change_velocity']*100:.1f}%" if analysis['metrics']['change_velocity'] > 0 else "Stable")
    ...
    if trend and trend.get('status') != 'insufficient_data':
        ...
        with col2:
            risk_color = "🟢" if trend.get('risk') == 'low' else "🟡" if trend.get('risk') == 'medium' else "🔴"
            st.metric("Risk Level", f"{risk_color} {trend.get('risk', 'unknown').title()}")
            st.metric("Recommendation", trend.get('recommendation', 'No recommendation')[:50] + "...")
    ...
    if analysis.get('significant_changes'):
        ...
        st.dataframe(
            changes_df[['from_version', 'to_version', 'old_similarity', 'new_similarity', 
                       'similarity_change', 'timestamp']].style.format({
                           'old_similarity': '{:.1%}',
                           'new_similarity': '{:.1%}',
                           'similarity_change': '{:.1%}'
                       }),
...
def render_smart_detection_ui(version_manager: VersionManager, doc_name: str):
    ...
    st.dataframe(
        df[['from_version', 'to_version', 'similarity_ratio', 'change_percentage', 
           'suspicious_score', 'risk_level']].style.format({
               'similarity_ratio': '{:.1%}',
    ...
    for idx, row in df.iterrows():
        if row['suspicious_score'] > 0.4:
            with st.expander(f"⚠️ Version {row['from_version']} → {row['to_version']} - {row['risk_level'].title()} Risk", expanded=True):
...
def render_global_version_dashboard(version_manager: VersionManager):
    ...
    col3.metric("Average Versions per Document", f"{total_versions/len(documents):.1f}")
    ...
    for doc in documents:
        ...
        doc_data.append({
            ...
            'Last Updated': versions[-1].timestamp.strftime('%Y-%m-%d %H:%M') if versions else 'Never',
...
def initialize_version_control():
    ...
    if 'version_tabs' not in st.session_state:
        st.session_state['version_tabs'] = {
            'current_tab': 'Version History'
        }
    ...
    with tab1:
        ...
        if documents:
            doc_name = st.selectbox(
                "Select Document",
                options=documents,
                key="version_history_doc_selector"
            )
            render_version_control_ui(version_manager, doc_name)
        else:
            st.info("No documents with version history. Upload documents to start tracking versions.")
    ...
    with tab3:
        ...
        if documents and len(version_manager.get_version_history(documents[0])) >= 2:
            doc_name = st.selectbox(
                "Select Document",
                options=documents,
                key="smart_detection_doc_selector"
            )

...
class SearchEngine:
    ...
    def search_hybrid(self, query: str, query_embedding: np.ndarray = None,
        ...
        if query_embedding is not None:
            semantic_results = self.search_semantic(query_embedding, filters, max_results)
        ...
        for doc_name, data in combined.items():
            if data['count'] == 1:
                ...
                # Hybrid score
                score = (semantic_weight * data['semantic_score'] + 
...
def render_search_filters(filter_builder: SmartFilterBuilder):
    ...
    author = st.text_input(
        ...
        key="author_filter_input"
    )
    if author:
        author_operator = st.selectbox(
            "Author operator:",
            ['eq', 'contains'],
            key="author_op_select"
        )
...
def render_search_results(results: List[SearchResult]):
    ...
    for idx in range(start_idx, end_idx):
        ...
        with st.expander(
            ...
            with col1:
                if st.button("📄 View Document", key=f"view_{result.document_name}_{idx}"):
                    st.session_state['selected_doc'] = result.document_name
                    st.rerun()
            with col2:
                if st.button("📝 Annotate", key=f"annotate_{result.document_name}_{idx}"):
...
def render_advanced_search_dashboard():
    ...
    # Execute search
    if st.session_state.get('active_search', False) or (query and st.session_state.get('search_query')):
        ...
        with st.spinner("Searching..."):
            if search_type == 'full_text':
                ...
                results = search_engine.search_hybrid(search_query, query_embedding, filters)

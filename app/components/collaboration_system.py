...
class ReviewWorkflow:
    ...
    def submit_review(self, reviewer_id: str, decision: str, comments: str = ""):
        ...
        self.add_timeline_event('review_submitted', reviewer_id, {
            'decision': decision,
            'comments': comments
        })
    ...
    def _update_status(self):
        ...
        elif len(decisions) >= len(self.reviewers) and all(d == 'approved' for d in decisions):
...
class AnnotationManager:
    ...
    def add_annotation(self, doc_name: str, user_id: str, content: str, 
                      annotation_type: str = 'comment') -> DocumentAnnotation:
...
class WorkflowManager:
    ...
    def get_workflow_status_summary(self) -> dict:
        ...
        return {
            ...
            'pending': len([w for w in self.workflows.values() if w.status == 'pending']),
            'in_review': len([w for w in self.workflows.values() if w.status == 'in_review']),
            'approved': len([w for w in self.workflows.values() if w.status == 'approved']),
            'rejected': len([w for w in self.workflows.values() if w.status == 'rejected']),
            'needs_changes': len([w for w in self.workflows.values() if w.status == 'needs_changes'])
        }
...
class ActivityManager:
    ...
    def get_document_activity(self, doc_name: str) -> dict:
        ...
        return {
            ...
            'users': [{'username': s.username, 'action': s.current_action} for s in active_users],
            ...
            'reviewing': len([u for u in active_users if u.current_action == 'reviewing']),
            'commenting': len([u for u in active_users if u.current_action == 'commenting'])
        }
...
class ReviewSystem:
    ...
    def get_review_summary(self, doc_name: str) -> dict:
        ...
        return {
            ...
            'annotation_summary': self.annotation_manager.get_annotation_summary(doc_name),
    ...
    def get_user_review_dashboard(self, user_id: str) -> dict:
        ...
        my_decisions = [d for d in self.decision_history if d.get('reviewer_id') == user_id]
...
def render_annotation_ui(review_system: ReviewSystem, doc_name: str, user_id: str):
    ...
    if filter_type != 'All':
        filtered_annotations = [a for a in filtered_annotations if a.annotation_type == filter_type]
        ...
        with st.expander(
            ...
            if ann.replies:
                ...
                for reply in ann.replies:
                    st.markdown(f"- {reply['user_id']}: {reply['content']} "
                               f"({reply['created_at']})")
            
            # Reply form
            reply_text = st.text_input(
                "Add reply:",
                key=f"reply_{ann.id}_{doc_name}"
            )
            if st.button("Reply", key=f"reply_btn_{ann.id}_{doc_name}"):
                if reply_text.strip():
                    review_system.annotation_manager.add_reply(ann.id, user_id, reply_text)
            ...
            with col1:
                if not ann.resolved:
                    if st.button("✅ Resolve", key=f"resolve_{ann.id}_{doc_name}"):
                        review_system.annotation_manager.resolve_annotation(ann.id, user_id)
                        st.rerun()
            with col2:
    ...
    st.caption(f"Total: {summary['total']} | "
        ...
        with st.expander("📋 Create Review Workflow", expanded=True):
            ...
            if st.button("Create Workflow", key=f"create_wf_{doc_name}"):
                if selected_reviewers:
                    ...
                    st.success(f"✅ Workflow created with {len(selected_reviewers)} reviewers!")
                    st.rerun()
                else:
    ...
    if workflows:
        for workflow in workflows:
            with st.expander(
                ...
                with col2:
                    st.metric("Reviewers", f"{len(workflow.decisions)}/{len(workflow.reviewers)}")
                with col3:
                ...
                for reviewer in workflow.reviewers:
                    ...
                    if reviewer in workflow.decisions:
                        ...
                        status = f"✅ {decision.title()}" if decision == 'approved' else f"❌ {decision.title()}"
    ...
    if dashboard['pending_workflows']:
        ...
        for wf in dashboard['pending_workflows']:
            with st.expander(f"📄 {wf['doc_name']} - {wf['status'].upper()}"):
                ...
                st.markdown(f"**Decisions:** {len(wf['decisions'])}/{len(wf['reviewers'])}")
...
def render_activity_ui(review_system: ReviewSystem, doc_name: str):
    ...
    if activity['users']:
        ...
        for user in activity['users']:
            icon = "👁️" if user['action'] == 'viewing' else "✏️" if user['action'] == 'editing' else "📝"

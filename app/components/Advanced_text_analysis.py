...
class TextAnalysisEngine:
    ...
    def analyze_document(self, content: str, doc_name: str = None) -> LinguisticFeatures:
        ...
        # Document complexity
        complexity_score = self._calculate_complexity_score(readability, style, pos_distribution)
    ...
    def _calculate_style_metrics(self, text: str, words: List[str], sentences: List[str]) -> StyleMetrics:
        ...
        nominalization_count = len(re.findall(nominalization_pattern, text, re.IGNORECASE))
    ...
    def _calculate_style_consistency(self, words: List[str], sentences: List[str]) -> float:
        ...
        # Normalize variances
        max_variance = 100
        consistency = 1 - (min(length_variance, max_variance) + min(word_variance, max_variance)) / (2 * max_variance)
    ...
    def _get_vocabulary_profile(self, words: List[str]) -> Dict[str, Any]:
        ...
        freq_distribution.update({
            'simple_word_ratio': simple_words / total_words if total_words > 0 else 0,
    ...
    def compare_linguistic_features(self, features_a: LinguisticFeatures, 
        ...
        comparison['similarity_scores']['readability'] = 1 - abs(read_a.flesch_reading_ease - read_b.flesch_reading_ease) / 100
        
        # Compare style
        style_a = features_a.style
        style_b = features_b.style
        style_similarity = 1 - abs(style_a.vocabulary_richness - style_b.vocabulary_richness)
        comparison['similarity_scores']['style'] = min(style_similarity, 1)
        
        # Compare vocabulary
        vocab_sim = 1 - abs(features_a.unique_words - features_b.unique_words) / max(features_a.unique_words, features_b.unique_words)
        comparison['similarity_scores']['vocabulary'] = vocab_sim
        
        # Compare complexity
        comp_sim = 1 - abs(features_a.document_complexity_score - features_b.document_complexity_score)
        comparison['similarity_scores']['complexity'] = comp_sim
        
        # Overall similarity
        overall = sum(comparison['similarity_scores'].values()) / len(comparison['similarity_scores'])
        comparison['summary']['overall_similarity'] = overall
        comparison['summary']['risk_level'] = 'high' if overall > 0.7 else 'medium' if overall > 0.4 else 'low'
        
        # Differences
        comparison['differences'] = {
            'readability': abs(read_a.flesch_reading_ease - read_b.flesch_reading_ease),
            'vocabulary_richness': abs(style_a.vocabulary_richness - style_b.vocabulary_richness),
            'avg_sentence_length': abs(read_a.avg_sentence_length - read_b.avg_sentence_length),
            'word_complexity': abs(style_a.avg_word_complexity - style_b.avg_word_complexity)
        }
...
def display_analysis_results(features: LinguisticFeatures, doc_name: str):
    ...
    with detail_tab3:
        ...
        if features.vocabulary_profile:
            ...
            col3.metric("Simple Words Ratio", f"{profile.get('simple_word_ratio', 0):.3f}")
            
            if profile.get('most_common'):
                st.subheader("📝 Most Common Words")
                common_df = pd.DataFrame(profile['most_common'], columns=['Word', 'Frequency'])
    ...
    if st.button("📊 Analyze All Documents Style"):
        with st.spinner("Analyzing writing styles..."):
            ...
            if results:
                ...
                # Visualization
                style_metrics = ['Vocab Richness', 'Lexical Diversity', 'Style Consistency']
                fig = go.Figure()
...
def render_comparison_tab(analyzer: TextAnalysisEngine):
    ...
    with col2:
        doc_b = st.selectbox("Document B", [d for d in documents.keys() if d != doc_a], key="comp_b")
    ...
    if st.button("🔍 Compare Documents", type="primary"):
        with st.spinner("Comparing documents..."):
            ...
            col1, col2, col3 = st.columns(3)
            col1.metric("Overall Similarity", f"{comparison['summary']['overall_similarity']:.1%}")
            ...
            fig.add_trace(go.Bar(
                ...
                marker_color=['#4CAF50' if v > 0.6 else '#FF9800' if v > 0.3 else '#F44336' 
            ...
            for key, value in diffs.items():
                if value > 0.3:
                    st.warning(f"**{key.replace('_', ' ').title()}**: {value:.2f} difference")
                else:
                    st.info(f"**{key.replace('_', ' ').title()}**: {value:.2f} difference")

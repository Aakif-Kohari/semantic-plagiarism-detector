...
class PatternRecognitionEngine:
    ...
    def _extract_features(self, documents: Dict[str, str], 
        ...
        for i, doc_name in enumerate(doc_names):
            ...
            # Average word length
            if words:
                features['avg_word_length'][doc_name] = sum(len(w) for w in words) / len(words)
    ...
    def _detect_structural(self, documents: Dict[str, str]) -> List[PlagiarismPattern]:
        ...
        for doc_name, content in documents.items():
            ...
            if para_lengths:
                ...
                doc_patterns[doc_name] = {
                    ...
                    'length_variance': np.var(para_lengths) if len(para_lengths) > 1 else 0
                }
        ...
        for i in range(len(doc_names)):
            for j in range(i + 1, len(doc_names)):
                ...
                para_diff = abs(pattern1['paragraph_count'] - pattern2['paragraph_count'])
                para_sim = 1 - (para_diff / max(pattern1['paragraph_count'], pattern2['paragraph_count']))
                
                # Compare average length
                len_diff = abs(pattern1['avg_paragraph_length'] - pattern2['avg_paragraph_length'])
                max_len = max(pattern1['avg_paragraph_length'], pattern2['avg_paragraph_length'])
                    ...
                    if overlap > 0.5:
                        ...
                        pattern = PlagiarismPattern(
                            ...
                            features={
                                ...
                                'shared_citations': list(citations1.intersection(citations2))
                            }
                        )
                        patterns.append(pattern)
    ...
    def _detect_hybrid(self, documents: Dict[str, str], 
        ...
        for i in range(len(doc_names)):
            for j in range(i + 1, len(doc_names)):
                ...
                sem_sim = similarity_matrix[i][j] if i < len(similarity_matrix) and j < len(similarity_matrix[i]) else 0
...
class PatternEvolutionTracker:
    ...
    def _update_trend(self, pattern_id: str):
        ...
        recent = history[-5:]
        if len(recent) >= 2:
            freq_change = (recent[-1]['data'].get('frequency', 0) - 
                          recent[0]['data'].get('frequency', 0))
...
def render_detection_tab(pattern_engine: PatternRecognitionEngine):
            ...
            if patterns:
                ...
                for pattern in patterns:
                    with st.expander(f"📌 {pattern.name}", expanded=False):
                        ...
                        with col3:
                            st.metric("First Detected", pattern.first_detected.strftime('%Y-%m-%d'))
    ...
    if pattern_engine.patterns:
        ...
        # Pattern summary
        pattern_types = Counter(p.pattern_type for p in pattern_engine.patterns.values())
...
def render_predictions_tab(pattern_engine: PatternRecognitionEngine):
    ...
    if len(pattern_engine.patterns) >= 5:
        ...
        for pattern in pattern_engine.patterns.values():
            training_data.append({
                ...
                'word_count': sum(len(d.split()) for d in pattern.documents) / len(pattern.documents) if pattern.documents else 0,
    ...
    if pattern_engine.predictions:
        ...
        for pred in list(pattern_engine.predictions.values())[-5:]:
            st.markdown(f"- **{pred.document_name}**: {pred.risk_level.upper()} ({pred.predicted_risk_score:.1%})")

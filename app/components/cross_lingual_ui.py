    ...
    with st.form("cross_lingual_config"):
        ...
        with col1:
            ...
            languages = st.multiselect(
                ...
                default=["en", "es", "fr", "de"],
                format_func=lambda x: f"{LANGUAGE_NAMES.get(x, get_language_name(x))} ({x})"
            )
    ...
    for f in uploaded_files:
        ...
        if manual_lang:
            lang = st.selectbox(f"Language for {f.name}", list(LANGUAGE_NAMES.keys()),
                format_func=lambda x: LANGUAGE_NAMES.get(x, get_language_name(x)), key=f"lang_{f.name}")
        else:
...
def _render_results():
    ...
    lang_data = [{"Language": LANGUAGE_NAMES.get(k, get_language_name(k)), "Count": v} for k, v in data["language_distribution"].items()]
    if lang_data:
    ...
    if data["matches"]:
        ...
        for i, match in enumerate(data["matches"][:10]):
            ...
            with st.expander(
                ...
                c1, c2 = st.columns(2)
                with c1:
                    st.markdown(f"**📄 {match['source_doc']}** ({LANGUAGE_NAMES.get(match['source_lang'], get_language_name(match['source_lang']))})")
                    st.info(match["source_chunk"][:500] if match["source_chunk"] else "(empty)")
                with c2:
                    st.markdown(f"**📄 {match['target_doc']}** ({LANGUAGE_NAMES.get(match['target_lang'], get_language_name(match['target_lang']))})")
...
def render_language_badge(lang_code: str, show_name: bool = True) -> str:
    ...
    colors = {
        "en": "#3B82F6", "es": "#F59E0B", "fr": "#10B981", "de": "#EF4444",
        "it": "#8B5CF6", "pt": "#EC4899", "nl": "#06B6D4", "ru": "#6B7280",
        "zh": "#EF4444", "ja": "#F472B6", "ko": "#8B5CF6", "ar": "#059669",
        "hi": "#F97316", "tr": "#3B82F6", "pl": "#F472B6"
    }
...
def render_translation_indicator(is_translated: bool, lang_code: Optional[str] = None) -> str:
...
def render_cross_lingual_settings() -> bool:
    ...
    enabled = st.toggle("🌐 Enable Cross-Lingual Detection", value=st.session_state.get("cross_lingual_mode_toggle", False), key="cross_lingual_mode_toggle")
    return enabled

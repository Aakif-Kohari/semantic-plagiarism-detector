...
def render_stopword_manager_ui() -> None:
    ...
    with col2:
        st.write("")
        if st.button("➕ Add Stopwords", key="add_stopwords_btn", use_container_width=True):
    ...
    if manager.custom:
        ...
        st.caption(f"Showing {start_idx + 1}-{end_idx} of {len(custom_list)} custom stopwords")
        ...
        if total_pages > 1:
            ...
            with col3:
                if st.button("Next ▶️", key="stopword_next", disabled=(page >= total_pages)):
                    st.session_state["stopword_page"] = page + 1
                    st.rerun()
    ...
    with st.expander("👁️ View All Stopwords", expanded=False):
        ...
        if search:
            ...
            st.code("\n".join(filtered[:100]) + ("\n..." if len(filtered) > 100 else ""), language=None)
        else:
            st.caption(f"Total: {len(all_stopwords)} stopwords")
            st.code("\n".join(all_stopwords[:100]) + ("\n..." if len(all_stopwords) > 100 else ""), language=None)

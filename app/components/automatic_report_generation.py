...
class ReportGenerator:
        ...
        default_templates = [
            {
                ...
                'sections': ['Overview', 'Key Metrics', 'Risk Summary', 'Recommendations'],
                'style_config': {'color_scheme': 'professional', 'include_charts': True},
            ...
            },
            {
                ...
                'sections': ['Overview', 'Document Analysis', 'Similarity Matrix', 'Flagged Cases', 
                            'Trend Analysis', 'Recommendations', 'Appendix'],
            ...
            },
            {
                ...
                'sections': ['Compliance Overview', 'Policy Violations', 'Actions Taken', 'Audit Trail'],
    ...
    def _get_style_css(self, style_config: Dict) -> str:
        ...
        colors = {
            ...
            'analytical': {'primary': '#263238', 'secondary': '#455a64', 'bg': '#eceff1'},
    ...
    def _generate_insights(self, data: Dict, template: ReportTemplate) -> List[ReportInsight]:
        ...
        trends = data.get('trends', {})
        for key, value in trends.items():
            if value.get('direction') in ['up', 'down'] and abs(value.get('change', 0)) > 0.1:
                insights.append(ReportInsight(
                    id=str(uuid.uuid4()),
                    report_id="",
                    type='trend',
                    ...
                    severity='medium',
                    confidence=0.75,
                    data={'direction': value.get('direction'), 'change': value.get('change', 0)}
                ))
    ...
    def get_report_stats(self) -> Dict:
        ...
        return {
            ...
            'avg_size': sum(r.file_size for r in self.reports.values()) / total if total else 0
        }
        ...
        if template_id:
            ...
            with st.spinner("Generating report..."):
                ...
                if report:
                    ...
                    # Export
                    report_bytes = report_generator.export_report(report.id, format_enum)
...
def render_templates_tab(report_generator: ReportGenerator):
    ...
    with st.expander("➕ Create Custom Template", expanded=False):
        ...
        with col2:
            sections = st.text_area("Sections (one per line)", 
                                   placeholder="Overview\nKey Metrics\nRisk Summary\nRecommendations")
            format_enum = st.selectbox("Default Format", [f.value for f in ExportFormat])
    ...
    if jobs:
        for job in jobs:
            with st.expander(f"📅 {job.name}", expanded=False):
                st.markdown(f"**Template:** {report_generator.templates.get(job.template_id, {}).name}")
                ...
                if job.last_run:
                    st.markdown(f"**Last Run:** {job.last_run.strftime('%Y-%m-%d %H:%M')}")
                if job.next_run:
                    st.markdown(f"**Next Run:** {job.next_run.strftime('%Y-%m-%d %H:%M')}")
...
def render_history_tab(report_generator: ReportGenerator):
    ...
    if reports:
        ...
        for report in reports[-20:]:
            with st.expander(f"📄 {report.title} - {report.generated_at.strftime('%Y-%m-%d %H:%M')}"):
                st.markdown(f"**ID:** {report.id}")
                st.markdown(f"**Template:** {report_generator.templates.get(report.template_id, {}).name}")

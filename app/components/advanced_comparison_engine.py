...
class AdvancedComparisonEngine:
    ...
    def compare_documents(self, text_a: str, text_b: str, 
                          doc_a_name: str = "Document A",
                          doc_b_name: str = "Document B") -> DocumentComparisonResult:
    ...
    def _calculate_lexical_similarity(self, text_a: str, text_b: str) -> float:
        ...
        set_a = set(tokens_a)
        set_b = set(tokens_b)
        jaccard = len(set_a.intersection(set_b)) / len(set_a.union(set_b)) if set_a.union(set_b) else 0.0
    ...
    def _calculate_structural_similarity(self, text_a: str, text_b: str) -> float:
        ...
        count_similarity = 1 - abs(len(paras_a) - len(paras_b)) / max(len(paras_a), len(paras_b))
        ...
        if lengths_a and lengths_b:
            ...
            length_similarity = 1 - abs(avg_a - avg_b) / max(avg_a, avg_b) if max(avg_a, avg_b) > 0 else 0
        else:
    ...
    def _calculate_stylistic_similarity(self, text_a: str, text_b: str) -> float:
        ...
        for key in features_a.keys():
            if key in features_b:
                if key in ['avg_word_length', 'avg_sentence_length', 'avg_paragraph_length']:
        ...
        for metric in metrics:
            if metric.score >= 0.8:
                if metric.name == "Lexical Similarity":
                    recommendations.append("High lexical similarity detected. Review for direct copying.")
                elif metric.name == "Semantic Similarity":
                    recommendations.append("High semantic similarity detected. Check for paraphrasing.")
                elif metric.name == "Structural Similarity":
                    recommendations.append("Document structure is very similar. Verify source.")
                elif metric.name == "Stylistic Similarity":
                    recommendations.append("Writing style is highly similar. Check if same author.")
                elif metric.name == "Citation Similarity":
                    recommendations.append("Citation patterns match closely. Verify references.")
                elif metric.name == "Paraphrase Detection":
                    recommendations.append("Significant paraphrasing detected. Deep review needed.")
...
def render_comparison_engine_ui(comparison_engine: AdvancedComparisonEngine):
    ...
    with col2:
        doc_b = st.selectbox("Document B:", [d for d in documents if d != doc_a], key="comp_doc_b")
...
def display_comparison_results(result: DocumentComparisonResult):
    ...
    fig.add_trace(go.Bar(
        ...
        name='Score',
        marker_color=['#4CAF50' if m.is_passed() else '#FF9800' for m in result.metrics]
    ))

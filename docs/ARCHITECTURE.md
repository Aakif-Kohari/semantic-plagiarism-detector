...
flowchart LR
    ...
    B -->|Non-reference language| C{Cache Check}
    B -->|Reference language| E[Embedding Generation]
    C -->|Cache hit| D[Cached Translation]
    C -->|Cache miss| F[Back-Translation]
    F --> G[Store Translation in Cache]
    G --> D
    D --> E
...
2. **Cache Check**: If the chunk's language differs from the corpus reference language, the system checks the translation cache (e.g. Redis, via `src/utils/redis_cache.py`) for a previously computed translation, keyed by a hash of the chunk text and target language.
3. **Translation (Back-Translation)**: On a cache miss, the chunk is back-translated into the reference language using the configured translation backend, and the result is stored in the cache for future reuse.

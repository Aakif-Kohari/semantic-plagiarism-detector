...
class TestUIExceptionHandler:
    ...
    def test_ui_exception_handler_reraises_stop_exception(self):
        ...
        with pytest.raises(st.runtime.scriptrunner.StopException), patch("streamlit.error") as mock_st_error:
            component_with_stop()
    ...
    def test_ui_exception_handler_catches_generic_exception(self):
        ...
        with patch("streamlit.error") as mock_st_error, patch("app.state_manager.logger.error") as mock_log_error:
            result = failing_component()
            assert result is None
            mock_st_error.assert_called_once_with("⚠️ Failed to load component: FailingComponent")

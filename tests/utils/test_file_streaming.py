...
from src.utils.file_streaming import stream_upload_file_to_disk, DEFAULT_MAX_UPLOAD_BYTES, CHUNK_SIZE
...
async def test_custom_chunk_size_successful():
    ...
    try:
        with patch.object(mock_file, 'read', wraps=mock_file.read) as spy_read:
            temp_path = await stream_upload_file_to_disk(mock_file, chunk_size=custom_chunk_size)
            assert os.path.exists(temp_path)
...
async def test_maximum_bytes_exactly_equal(exactly_chunk_size_file):
    ...
    temp_path = ""
    try:
        temp_path = await stream_upload_file_to_disk(exactly_chunk_size_file, max_bytes=CHUNK_SIZE)
        assert os.path.exists(temp_path)
        assert os.path.getsize(temp_path) == CHUNK_SIZE
    finally:
        if temp_path and os.path.exists(temp_path):
...
async def test_file_writing_disk_full():
    ...
    with patch("tempfile.NamedTemporaryFile", return_value=FaultyTempFile(delete=False)):
        with pytest.raises(HTTPException) as exc_info:
...
@pytest.mark.parametrize("file_size,max_limit", [
    (10, 10),
    (1024, 1024),
    (12435, 12435),
    (65536, 65536),
...
async def test_precise_limit_matching(file_size, max_limit):
    ...
    temp_path = ""
    try:
        temp_path = await stream_upload_file_to_disk(mock_file, max_bytes=max_limit, chunk_size=8192)
        assert os.path.exists(temp_path)
...
async def test_precise_limit_exceeding(file_size, max_limit):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await stream_upload_file_to_disk(mock_file, max_bytes=max_limit, chunk_size=8192)
...
async def test_unlink_fails_on_internal_server_error():
    ...
    with patch("tempfile.NamedTemporaryFile", side_effect=RuntimeError("Arbitrary OS Error!")):
...
class IntFileMock:
    ...
    async def seek(self, offset): pass
    async def read(self, size=-1):

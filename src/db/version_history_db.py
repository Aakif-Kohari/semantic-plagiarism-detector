...
def initialize_version_history_db(db_path: Optional[Path] = None) -> None:
    ...
    logger.info("Version history database initialized at %s", db_path or DEFAULT_DB_PATH)
    ...
    try:
        with get_connection(db_path) as conn:
            ...
            conn.execute(
                ...
                """,
                (
                    document_hash, user_id, assignment_id, 
                    next_version, parent_hash, datetime.utcnow().isoformat()
                )
            )
            return next_version

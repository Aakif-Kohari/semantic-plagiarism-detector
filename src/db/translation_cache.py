...
def purge_expired_translation_cache(days_old: int = 60) -> int:
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            conn.commit()
            logger.info(f"Purged {deleted_count} expired translation cache entries older than {days_old} days.")
...
def get_translation_cache_stats() -> dict:
    ...
    try:
        with sqlite3.connect(DB_PATH) as conn:
            ...
            cursor.execute(
                "SELECT CAST(JULIANDAY('now') - JULIANDAY(MIN(created_at)) AS INTEGER) FROM translation_cache"
            )

...
def build_similarity_cache_key(session_id: Union[str, Path], *, use_hybrid: bool) -> str:
    ...
    Converts pathing logic using pathlib.Path(p).as_posix() for cross-platform support (Issue #2939).
    """
    sid_str = str(session_id)
    if "\\" in sid_str or isinstance(session_id, Path):
        sid_str = Path(sid_str).as_posix()

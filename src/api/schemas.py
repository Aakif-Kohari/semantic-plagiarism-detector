...
class HealthzResponse(BaseModel):
    """Response schema for load balancer healthz monitoring."""
    status: str = Field(..., description="Health status indicator")
    db_size_bytes: int = Field(..., description="Total SQLite database size in bytes")
    db_size_mb: float = Field(..., description="Total SQLite database size in megabytes")


class FlaggedChunkMatch(BaseModel):
    """Schema for individual paragraph or text chunk match pairs."""
    uploaded_chunk: str = Field(..., description="Original text chunk from uploaded document")
    matched_chunk: str = Field(..., description="Matched text chunk from target document")
    similarity_score: float = Field(..., description="Cosine similarity score for chunk pair")


class MatchedDocument(BaseModel):
    """Schema for document matching results in plagiarism detection."""
    filename: str = Field(..., description="Filename of matched target document in corpus")
    document_similarity_score: float = Field(..., description="Overall document similarity score")
    max_chunk_similarity_score: float = Field(..., description="Maximum chunk similarity score")
...
class SimilarityCheckResponse(BaseModel):
    ...
    plagiarism_flagged: bool = Field(..., description="Whether plagiarism was flagged based on threshold")
    threshold_used: float = Field(..., description="Similarity threshold configured for scan")
    overall_document_similarity: float = Field(..., description="Highest overall document similarity score")
    max_chunk_similarity: float = Field(..., description="Highest chunk-level similarity score")
    matched_documents_count: int = Field(..., description="Total count of matched corpus documents")

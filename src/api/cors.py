...
def validate_and_parse_origin(origin_str: str) -> tuple[str | None, str | None]:
    ...
    if "*" in origin_str:
        ...
        if len(parts) == 2 and parts[0].endswith("://"):
            ...
            raise ValueError(f"Wildcard '*' is only supported in the subdomain position (e.g., https://*.example.com): {origin_str}")
    ...
    if parsed.scheme not in ("http", "https"):
        raise ValueError(f"Unsupported scheme '{parsed.scheme}' in CORS origin: {origin_str}")

    if parsed.path or parsed.params or parsed.query or parsed.fragment:
        raise ValueError(f"CORS origin cannot contain paths or query parameters: {origin_str}")

...
class HybridScorer:
    ...
    def compute_lexical_scores(self, documents: List[str]) -> np.ndarray:
        ...
        logger.info(f"Computing lexical similarity matrix for {len(documents)} documents...")
        start_time = time.time()
    ...
    def compute_embedding_scores(self, documents: List[str]) -> Optional[np.ndarray]:
        ...
        # Check cache
        if (self.cached_embeddings is not None and 
            self.cached_documents == documents):
    ...
    def compute_hybrid_matrix(
        ...
        # Check cache
        if (self.cached_hybrid_matrix is not None and 
            self.cached_documents == documents):
            ...
            if embedding_scores is not None:
                ...
                hybrid_matrix = (alpha * lexical_scores) + ((1 - alpha) * embedding_scores)
                
                # Ensure diagonal is 1.0
                np.fill_diagonal(hybrid_matrix, 1.0)
                
                logger.info(f"Hybrid matrix shape: {hybrid_matrix.shape}")
                logger.info(f"Score range: [{hybrid_matrix.min():.4f}, {hybrid_matrix.max():.4f}]")
        ...
        if return_lexical:
            results.append(self.cached_tfidf_matrix if self.cache_matrix 
                          else self.compute_lexical_scores(documents))
        if return_embedding:
            results.append(self.cached_embeddings if self.cache_matrix 
    ...
    def get_statistics(self, documents: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        if documents:
            ...
            stats['vocabulary_size'] = len(self.vectorizer.vocabulary_)
            stats['sparsity'] = 1.0 - (tfidf_matrix.nnz / (tfidf_matrix.shape[0] * tfidf_matrix.shape[1]))
...
class TestHybridScorer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_embedding_model.encode.return_value = np.random.randn(len(self.documents), 128)
    ...
    def test_compute_lexical_scores(self):
        ...
        self.assertEqual(lexical_scores.shape, (len(self.documents), len(self.documents)))
    ...
    def test_compute_embedding_scores_with_model(self):
        ...
        self.assertEqual(embedding_scores.shape, (len(self.documents), len(self.documents)))
    ...
    def test_compute_hybrid_matrix_lexical_only(self):
        ...
        self.assertEqual(hybrid_matrix.shape, (len(self.documents), len(self.documents)))
    ...
    def test_get_top_k_similar(self):
        ...
        results = self.scorer.get_top_k_similar(
            self.query_doc,
            self.documents,
            k=3
        )
    ...
    def test_get_top_k_similar_batch(self):
        ...
        results = self.scorer.get_top_k_similar_batch(
            query_docs,
            self.documents,
            k=2
        )
    ...
    def test_performance_comparison(self):
        ...
        test_docs = [
            f"Document {i} with some random text content for testing" 
            for i in range(50)
        ]
...
def run_performance_benchmark():
    ...
    print("SUMMARY")
    print("="*60)
    print(f"{'Documents':<12} {'Old Time (s)':<15} {'New Time (s)':<15} {'Speedup':<10}")
    print("-"*60)

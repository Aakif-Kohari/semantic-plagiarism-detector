...
class RollingHash:
    ...
    def compute_window_hash(self, tokens: List[str], start: int) -> int:
        ...
        for i in range(self.config.ngram_size):
            ...
            hash_value = (hash_value * self.config.base + token_hash) % self.config.modulus
    ...
    def compute_rolling_hashes(self, tokens: List[str]) -> Iterator[int]:
        ...
        for i in range(k):
            ...
            current_hash = (current_hash * self.config.base + token_hash) % self.config.modulus
        ...
        for i in range(k, n):
            ...
            current_hash = (current_hash - old_token_hash * powers[k]) % self.config.modulus
            
            # Add the new token
            new_token_hash = self._hash_token(tokens[i])
            current_hash = (current_hash * self.config.base + new_token_hash) % self.config.modulus

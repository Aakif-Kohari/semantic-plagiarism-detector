...
from src.db.auth import add_user, init_db as init_auth_db
...
def seed_documents():
    ...
    logger.info(f"✓ Documents: {len(created_docs)} created / {len(recovered_docs)} recovered / {len(skipped_docs)} already present")
    
    if raw_texts:
        try:
            # Chunking
            chunked_docs = chunk_documents(
                raw_texts, chunk_size=500, chunk_overlap=50
            )
